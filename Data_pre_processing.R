...
CY_CAL <- fread("Z:\\External\\QVD_Protect\\Modeling\\Temp\\AVRA\\CALENDAR.csv", stringsAsFactors = FALSE)

## load Data:
PARTS <- fread('Z:\\External\\QVD_Protect\\Modeling\\Temp\\AVRA\\ML_PARTS_TEMP.csv', stringsAsFactors = FALSE)

# Check why we have the unique() function below
PARTS_FAM_MAP <- fread("Z:\\External\\QVD_Protect\\Modeling\\Temp\\AVRA\\ML_PARTS_FAMILY_TEMP.csv", stringsAsFactors = FALSE)
PARTS_FAM_MAP <- unique(PARTS_FAM_MAP)

# Check why we have the unique() function below
PARTS_AGE <- fread("Z:\\External\\QVD_Protect\\Modeling\\Temp\\AVRA\\ML_PARTS_AGE_TEMP.csv", stringsAsFactors = FALSE)
PARTS_AGE <- unique(PARTS_AGE)

LEAD_TIME <- fread("Z:\\External\\QVD_Protect\\Modeling\\Temp\\AVRA\\ML_LEAD_TIME.csv", stringsAsFactors = FALSE)

BACKLOGS_RAW <- fread("Z:\\External\\QVD_Protect\\Modeling\\Temp\\AVRA\\ML_BACKLOGS_TEMP.csv", stringsAsFactors = FALSE)

REVENUE_RAW <- fread("Z:\\External\\QVD_Protect\\Modeling\\Temp\\AVRA\\ML_REVENUE_TEMP.csv", stringsAsFactors = FALSE)
...
CATA_17_19_LIST <- fread('Z:\\External\\QVD_Protect\\Modeling\\Temp\\AVRA\\MASTER_FC_CUST_17-19.csv')

LEAD_TIME <- fread('Z:\\External\\QVD_Protect\\Modeling\\Temp\\AVRA\\ML_LEAD_TIME.csv')
...
OLD_NEW_MPN_MAP <- fread("Z:\\External\\QVD_Protect\\Modeling\\Temp\\AVRA\\MB_BCM_TO_CY_PARTS_MAPPING.csv", stringsAsFactors = FALSE)
## clean up MPNS:
...
MPN_PRICE_OPT <- fread("Z:\\External\\QVD_Protect\\Modeling\\Temp\\AVRA\\ML_MPN_PRICE_OPT.csv", stringsAsFactors = FALSE)

MPN_PRICE_OPT_MODEL <- unique(MPN_PRICE_OPT[,.(MKT_PART_NUM,FISCAL_YEAR_QUARTER,JERRY_MPN_VARIATION_BIN,
                                               #JERRY_SIZE_Q_BIN,
                                               PRODUCT_AGE_BIN)])

JERRY_PRICE_OPT <- fread("Z:\\External\\QVD_Protect\\Modeling\\Temp\\AVRA\\ML_JERRY_PRICE_OPT.csv", stringsAsFactors = FALSE)
...
MPN_WAFER_MAP <- fread("Z:\\External\\QVD_Protect\\Modeling\\Temp\\AVRA\\ML_PARTS_WAFERS_MAP_TEMP.csv", stringsAsFactors = FALSE)
